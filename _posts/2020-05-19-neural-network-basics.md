...
In this example, we add up the corresponding weight if certain item interest us. The threshold value can be also parameter depending on how easily it can get the output to be true (1).

### Sigmoid Neuron

*Sigmoid neuron* takes the float value from 0 and 1. When changing weight of some neuron, the effect will be continuous rather than be sharp with perceptron. In the video of *3blue1brown*, the model uses sigmoid neuron. The word *sigmoid* here represents *sigmoid function*, which will convert any value to a range from 0 to 1.
